...
#[derive(Debug)]
pub(crate) enum ByteBlockEncoder {
    Raw(Cursor<Vec<u8>>),
...
/// block.
#[derive(Debug)]
pub(crate) struct ByteBlockWriter {
    /// the encoder for the byte-block
...
impl ByteBlockWriter {
    ...
    pub(crate) fn new(encoding: AccountBlockFormat) -> Self {
        Self {
    ...
    pub(crate) fn write_pod<T: bytemuck::NoUninit>(&mut self, value: &T) -> IoResult<usize> {
    ...
    /// AccountMetaFlags.
    pub(crate) fn write_optional_fields(
        &mut self,
    ...
    pub(crate) fn finish(self) -> IoResult<Vec<u8>> {
        match self.encoder {
...
pub(crate) struct ByteBlockReader;
...
pub(crate) fn read_pod<T: bytemuck::AnyBitPattern>(byte_block: &[u8], offset: usize) -> Option<&T> {
...
pub(crate) unsafe fn read_type<T>(byte_block: &[u8], offset: usize) -> Option<&T> {
...
impl ByteBlockReader {
    ...
    pub(crate) fn decode(encoding: AccountBlockFormat, input: &[u8]) -> IoResult<Vec<u8>> {
        match encoding {
