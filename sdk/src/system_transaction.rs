...
pub fn create_account(
    ...
    let instruction = solana_system_instruction_core::create_account(
        &from_pubkey,
        &to_pubkey,
        lamports,
        space,
...
pub fn allocate(
    ...
    let instruction = solana_system_instruction_core::allocate(&account_pubkey, space);
...
pub fn assign(from_keypair: &Keypair, recent_blockhash: Hash, program_id: &Pubkey) -> Transaction {
    let from_pubkey = from_keypair.pubkey();
    let instruction = solana_system_instruction_core::assign(&from_pubkey, program_id);
    let message = Message::new(&[instruction], Some(&from_pubkey));
...
pub fn transfer(
    ...
    let from_pubkey = from_keypair.pubkey();
    let instruction = solana_system_instruction_core::transfer(&from_pubkey, to, lamports);
    let message = Message::new(&[instruction], Some(&from_pubkey));
    Transaction::new(&[from_keypair], message, recent_blockhash)
}
