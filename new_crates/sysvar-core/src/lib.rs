...
macro_rules! declare_sysvar_id(
    ($name:expr, $type:ty) => (
        ::solana_sdk_macro::program_declare_id_lite!($name);
        ...
        #[test]
        fn test_sysvar_id() {
            assert!($crate::is_sysvar_id(&id()), "sysvar::is_sysvar_id() doesn't know about {}", $name);
        }
    )
);
...
macro_rules! declare_deprecated_sysvar_id(
    ($name:expr, $type:ty) => (
        ::solana_sdk_macro::program_declare_deprecated_id_lite!($name);
